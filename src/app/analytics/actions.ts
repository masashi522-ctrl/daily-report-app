...
各見出しの内容は箇条書きを使わず、文章（段落）で書いてください。
体重データがある場合は【バイタル・健康状態】または【食事・水分の様子】の中で具体的な数値を引用して触れること。

【分量と具体性の基準（すべての見出しに適用）】:
・各見出しは3〜4文の段落にすること。1〜2文で終わらせないこと。ケアマネジャーが読んで、今月の様子が具体的に思い浮かぶ分量が必要です
・「特に変わりなくお過ごしです」「問題なく経過しております」といった一言で締めないこと。落ち着いて過ごされている月であっても、何がどう落ち着いていたのかを、データや様子に即して具体的に書くこと
・書く材料は、以下の観点から探すこと。すべてに触れる必要はないが、各見出しで最低1つは具体的な内容を含めること：
　－ 活動やレクリエーションへの参加の仕方、意欲、他の利用者や職員との関わり方
　－ 機能訓練への取り組み方、無理なく続けられている動作、負担が大きそうな動作
　－ バイタル・食事・水分・体重の数値から読み取れる傾向（月内での変化の有無。安定しているなら、その根拠となる数値とともに書くこと）
　－ 入浴・移動・排泄などの場面での介助量や、ご自身でできている部分
・ただし、実施回数の列挙（「入浴○回、機能訓練○回」など）で分量を稼ぐことは禁止です
・記録に無いことを推測で作文しないこと。事実として書ける材料が本当に乏しい項目のみ、無理に膨らませず簡潔にまとめること
...
・書く材料が少ないからといって、回数の列挙で文章を長くすることは絶対にしないこと。上記【分量と具体性の基準】の観点から、実際に観察できた内容で埋めること。
...
${hasRecords ? `・上記「現場の記録」がある場合はその内容をもとに、①デイで実際に観察された具体的な事実（日付・部位・状況など）、②介護職員・機能訓練指導員としての見解（原因の推測や状態の解釈）、③デイでの対応内容、④ご自宅での様子や対応について、ケアマネジャーに確認・共有をお願いする一文、の4つの要素を1つの段落の中に自然な流れで盛り込むこと。④は「ご自宅での○○について、ケアマネジャー様からもご確認いただけますと幸いです。」のように、具体的に何を確認してほしいかが分かる形で必ず含めること。【事実】【評価】のような見出しやラベルは付けず、普通の報告文として書くこと。` : forceDetailed ? `・回数の列挙で分量を稼ぐのではなく、活動への参加の様子や取り組み方、機能訓練での様子など、実際に観察できた具体的な内容を中心に記載すること。` : `・今月はこの利用者について、特記すべき出来事の記録はありません。ただし「いつも通り穏やかにお過ごしいただきました」のような定型文だけで終わらせないこと。上記【分量と具体性の基準】の観点から、活動への参加の様子や機能訓練での取り組み方など、実際に観察できた内容を3文程度で具体的に書くこと。口腔ケアには触れないこと。`}
${forceDetailed ? `
【この利用者は「詳しく報告する」指定です（全体に関わる最重要指示）】:
・状態に変化のあった方、またはケアプランの更新時期にあたる方として指定されています。デイでのご様子を、いつもより丁寧に詳しくお伝えしてください。
・各見出しは4〜6文とすること（上記【分量と具体性の基準】の3〜4文より、この指定を優先する）。
・「特に変わりありません」「問題なくお過ごしです」で済ませないこと。数値や記録に現れている範囲で、必ず具体的な観察内容を書くこと。
・通常の観点に加えて、次の点を分かる範囲で具体的にお伝えすること：
　－ 月の前半と後半でのご様子の違い（変化がなければ、変化がなかったことを根拠となる事実とともに）
　－ ご本人の表情・お言葉・意欲など、数値には表れないご様子
　－ 介助をさせていただいている場面と、ご自身でなさっている場面の具体的なご様子
　－ 機能訓練で取り組んでいただいている内容と、その際のご様子
・【気になる点・お伝えしたいこと】は、デイで気づかせていただいた点をお伝えしたうえで、ご家庭でのご様子について「お差し支えなければお教えいただけますと幸いです」とお願いする一文を添えること。気になる点が特にない場合は、無理に挙げず「今月は落ち着いてお過ごしいただいております」と率直にお伝えすること。
  ...
  try {
    ...
    for (const model of models.slice(0, 3)) {
      const completion = await client.chat.completions.create({
        ...
        max_tokens: forceDetailed ? 3000 : 2000,
        messages: [
