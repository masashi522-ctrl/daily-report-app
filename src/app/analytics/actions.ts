...
・すべての見出しについて、通常より踏み込んだ内容にすること。各見出しは3〜5文とすること（上記【分量と具体性の基準】の3〜4文より、この指定を優先する）。【ケアサービス・活動の様子】と【気になる点・お伝えしたいこと】は特に厚く書くこと。
・「特に変わりありません」「問題なくお過ごしです」で済ませないこと。数値や記録に現れている範囲で、必ず具体的な観察内容を書くこと。
・現場の記録が少ない場合でも、上記【分量と具体性の基準】に挙げた観点から書ける材料を探し、各観点について具体的に記述すること（実施回数の列挙は禁止）。
  ...
  try {
    ...
    for (const model of models.slice(0, 3)) {
      const completion = await client.chat.completions.create({
        ...
        max_tokens: forceDetailed ? 2500 : 2000,
        messages: [
