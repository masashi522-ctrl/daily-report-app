        ...
        content: [
          sourceBlock,
          {
            ...
            text: [
              ...
              facilityName && [
                '【対象事業所の絞り込み（最重要）】',
                `この計画書を作成している事業所は「${facilityName}」（通所介護）です。`,
                '画像がケアマネジャー作成の「居宅サービス計画書」等、複数の事業所・サービス種別（訪問介護、訪問看護、福祉用具貸与、他のデイサービス等）の行が混在する書式である場合、',
                `援助目標の表のうち「サービス内容」欄や「サービス種別」欄が「${facilityName}」または「通所介護」に対応する行のみを抽出してください。`,
                '他の事業所・他のサービス種別（訪問介護、訪問看護、福祉用具貸与、ショートステイ、他のデイサービス等）に関する行は絶対に含めないでください。',
                'サービス種別の記載が曖昧で判断できない行は、無理に含めず除外してください。',
                '（画像が単一の通所介護計画書で、他事業所の行が存在しない場合は、記入されている行をすべて対象としてください。）',
                '',
              ].join('\n'),
              '【援助目標の表について】',
              ...
              '4. 記入されている行のうち、上記の絞り込み条件に合致する行のみ抽出し、完全に空欄の行は含めないでください。',
              '',
              '【全体の注意事項】',
