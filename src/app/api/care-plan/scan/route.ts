        ...
        content: [
          ...sourceBlocks,
          {
            ...
            text: [
              ...
              '「生活全般の解決すべき課題（ニーズ）」「長期目標」「短期目標」のセルは、複数の援助内容行にまたがって罫線で結合されている（＝同じ課題に対して複数のサービスが紐づいている）ことが多いので、上下のセルの結合状態を確認し、正しい組み合わせを取り違えないようにしてください。',
              facilityName ? [
                `${facilityName}が提供するサービスは「通所介護」です。`,
                '書式Bの場合、「サービス種別」欄が「通所介護」となっている行のみを抽出してください（訪問介護・訪問看護・福祉用具貸与・短期入所生活介護など、通所介護以外のサービス種別の行は除外してください）。',
                '抽出する際は、その「通所介護」の行の「サービス内容」「頻度」と、その行が属する（罫線で結合された）生活課題・長期目標・短期目標を正しく組み合わせて1件として出力してください。',
