        ...
        content: [
          ...sourceBlocks,
          {
            ...
            text: [
              ...
              '【重複禁止・統合ルール（最重要）】「解決すべき課題（ニーズ）」と「長期目標」のセルは、複数の援助内容行にまたがって罫線で結合されていることがあります。その場合、それらの行は「同じ1つの課題」を共有しています。',
              '一方で「短期目標」は、ニーズ・長期目標が同じでも行ごとに異なる内容になっていることがあります（例: 同じ課題に対して短期目標が2つあり、それぞれに対応するサービス内容・頻度が別々になっている）。列ごとに実際にセルが結合されているかを個別に確認してください。',
              '同じ課題（ニーズ）・長期目標を共有する援助内容行が複数ある場合、出力するgoals配列にはその課題につき1件のみを出力し、issue・longTermGoalの文言を複数のgoal項目にまたがって重複させないでください。',
              'ただし短期目標・サービス内容・頻度が行ごとに異なる場合は、情報を失わないよう、1件のgoal項目の中でそれぞれ「①」「②」…の番号を先頭に付けて改行区切りで列挙し、shortTermGoal・serviceContent・frequencyの間で同じ番号同士が対応するようにしてください（①の短期目標には①のサービス内容・①の頻度が対応する、という形）。短期目標も完全に同じ場合は1つだけ出力してください。',
              facilityName ? [
                ...
                '対象の行が、上記の統合ルールにより同じ課題を共有する複数の通所介護行である場合は、それらも1件のgoal項目にまとめ、短期目標・サービス内容・頻度を番号付きで対応させて列挙してください。',
