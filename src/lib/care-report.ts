...
・利用者の氏名は書かないこと（書類の上部に別途印刷されます）。呼び方は「ご利用者」で統一すること（「ご本人」は使わないこと）
...
【社交辞令を書かないこと（重要）】
・あいさつ・前置き・結びの決まり文句は一切書かないこと。「ご報告いたします」「今後ともよろしくお願いいたします」「引き続き、安心してお過ごしいただけるよう努めてまいります」「今後もお気づきの点がございましたら、随時ご連絡いたします」などは書かないこと
・それぞれの見出しに該当する内容だけを書き、書くことが無ければ書かないこと。文量を満たすための一般論や決まり文句で埋めないこと
...
・同じ文や同じ語尾を続けて繰り返さないこと。1つの内容は1回だけ述べ、言い換えて文章を長くしないこと`
  ...
  // 機能訓練の様子は［機能訓練］の申し送りメモを材料にするため、それが無い月は書けることがない。
  // 実施だけしていて内容の記録が無い月に、ありそうな訓練内容を書かせないよう分けている。
  const hasTrainingNotes = stats.careNotes.some(n => n.label === '機能訓練')
...
【書き方の禁止事項（最重要）】
・見出しは「【活動の様子】」と「【機能訓練の様子】」の2つだけを使うこと。この2つ以外の【】付きの見出しや、「1．」「①」「■」のような番号や記号、題名、日付は書かないこと（題名と日付は書類の上部に別途印刷されます）
...
・「8月5日には」「8月25日と29日には」のように日付を挙げて出来事を並べないこと。ひと月を通してどうだったかをまとめて書くこと。日付を書いてよいのは、健康や安全に関わる出来事をお伝えするときだけです
・数字を使ってよいのは、上記の日付だけです。利用日数・欠席日数は書類の上部に別途印刷されるため、本文に書かないこと
・介護計画書の目標に書かれている内容（歩行距離・回数・できるようになりたい動作など）を、実際にあった出来事として書かないこと。事実として書いてよいのは「現場の記録」にある場面だけです
・1つの出来事は1つの段落だけに書くこと。「気になる点」で取り上げた出来事を、「デイでのご様子」で繰り返さないこと

【全体の構成】
報告書は「活動の様子」と「機能訓練の様子」の2つに分け、それぞれの見出しを付けて書くこと。
機能訓練の話は「機能訓練の様子」にだけ書き、「活動の様子」には混ぜないこと。

■ 【活動の様子】（この順に、段落を分けて書くこと）

1つ目の段落（デイでのご様子）
${hasRecords ? `・「現場の記録」を材料に、活動やレクリエーション・入浴・お食事などの場面でのご様子を${forceDetailed ? '4〜6文' : '3〜4文'}で総括すること
・機能訓練の場面はここには書かず、「【機能訓練の様子】」にまとめること
...
2つ目の段落（気になる点・お伝えしたいこと）
${hasRecords ? `・この段落に書いてよいのは、健康や安全に関わる記録（体調の変化、発熱、痛み、皮膚の異常、外傷、ふらつきや転倒、食事や服薬に関することなど）がある場合だけです
・そうした記録が無い月は、この段落そのものを書かないこと。活動の話を繰り返したり、決まり文句で埋めたりしないこと
・「デイでのご様子」で書いた活動・日付・場面を、この段落で言い換えて再登場させることは禁止です
...
・【事実】【評価】のような見出しやラベルは付けず、普通の報告文として書くこと` : `・お伝えする出来事はありません。この段落そのものを書かないこと`}
・ケアマネジャーがご判断される事柄（加算・要介護区分・サービス内容の見直しなど）には立ち入らないこと。あくまでデイでのご様子をお伝えするにとどめること

■ 【機能訓練の様子】
${hasTrainingNotes ? `・「現場の記録」のうち［機能訓練］と書かれた記録だけを材料に、どのような訓練に取り組まれたか、そのときのご本人の様子や意欲、ひと月を通した変化を${forceDetailed ? '3〜5文' : '2〜3文'}で書くこと
・［機能訓練］以外の記録（活動・入浴・お食事など）の場面をここに書かないこと。それらは「【活動の様子】」に書くこと
・記録を日付順に並べて羅列しないこと。似た内容はひとまとめにして、ひと月を通した傾向として書くこと
・「サービスが実施できなかった記録」に機能訓練の未実施がある場合のみ、その理由に簡潔に触れること${hasCarePlan ? `
・介護計画書の援助目標のうち機能訓練に関わるものがあれば、今月の取り組みがその目標にどうつながっているかが伝わるように書くこと。目標の文言をそのまま引き写さず、実際の様子として自然に言い換えること
・目標に関係する記録が無い月は、無理に結びつけず、記録にある取り組みをそのまま書くこと` : ''}` : `・今月は［機能訓練］の記録がありません。「今月は機能訓練の記録がございませんでした。」のように1文で率直に書くこと
・記録に無い訓練内容やご様子を、ありそうなこととして書き足すことを禁じます`}

---
出力は次の形にすること（2つの見出し以外に【】や記号は使わず、段落は空行で区切ること）:

【活動の様子】

（デイでのご様子の段落）

（気になる点・お伝えしたいことの段落 ※健康や安全に関わる記録が無ければ書かない）
...
const PHRASE_FIXES: [RegExp, string][] = [
  ...
  // 社交辞令は書かない方針のため、決まり文句は言い換えるのではなく取り除く
  [/お気遣いいただけますよう、?お願い申し上げます。?/g, ''],
  [/引き続き、?安心してお過ごしいただけるよう努めてまいります。?/g, ''],
  [/今後ともよろしくお願いいたします。?/g, ''],
  [/今後もお気づきの点がございましたら、?随時ご連絡いたします。?/g, ''],
